...
import { addVisits, queryVisits, removeVisits, updateVisits, queryVisitsVerboseName, queryVisitsListDisplay, queryVisitsDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateVisits(value, current_id);
      hide();
  ...
  const dateFieldList = ["created_at","updated_at","deleted_at","start_time","end_time"]
  const base_columns = [{
                             title: 'id',
  ...
  useEffect(() => {
    queryVisitsDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryVisitsListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryVisits, table_columns, 'visits-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索visits" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'visits-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryVisits({ ...params, sorter, filter })}
