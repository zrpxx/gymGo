...
import { addCoaches, queryCoaches, removeCoaches, updateCoaches, queryCoachesVerboseName, queryCoachesListDisplay, queryCoachesDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCoaches(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryCoachesDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCoachesListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCoaches, table_columns, '教练管理-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索教练管理" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '教练管理-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCoaches({ ...params, sorter, filter })}
