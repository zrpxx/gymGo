...
import { addEquipment, queryEquipment, removeEquipment, updateEquipment, queryEquipmentVerboseName, queryEquipmentListDisplay, queryEquipmentDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateEquipment(value, current_id);
      hide();
        ...
        hideInSearch: true,
                      ...
                      message: '设备位置为必填项',
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, positionForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, positionVerboseNameMap);
                            },
                      ...
                      message: '设备状态为必填项',
                             ...
                             valueEnum:{"1":"可用","2":"被预约","3":"正在使用","4":"故障"}
                        },
                          {
                              title: '操作',
                              dataIndex: 'option',
  ...
  useEffect(() => {
    queryEquipmentDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryEquipmentListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [positionForeignKeyList, setPositionForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryZones({all: 1}).then(value => {
                                     setPositionForeignKeyList(value);
                                });
                                }, []);
                                const [positionVerboseNameMap, setPositionVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryZonesVerboseName().then(value => {
                                    setPositionVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryEquipment, table_columns, '设备管理-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索设备管理" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '设备管理-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryEquipment({ ...params, sorter, filter })}
