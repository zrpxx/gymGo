...
import { addReserveEquipment, queryReserveEquipment, removeReserveEquipment, updateReserveEquipment, queryReserveEquipmentVerboseName, queryReserveEquipmentListDisplay, queryReserveEquipmentDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryCustomers, queryCustomersVerboseName} from '@/pages/AutoGenPage/CustomersList/service';import {queryEquipment, queryEquipmentVerboseName} from '@/pages/AutoGenPage/EquipmentList/service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addReserveEquipment({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateReserveEquipment(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeReserveEquipment(ids);
      hide();
                              ...
                              render: (text, record) => (
                                <>
                                  ...
                                  <EditOutlined title="编辑" className="icon" onClick={async () => {
                                   record.due_time = record.due_time === null ? record.due_time : moment(record.due_time);
  ...
  useEffect(() => {
    queryReserveEquipmentDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryReserveEquipmentListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryReserveEquipment, table_columns, '设备预约-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索设备预约" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '设备预约-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryReserveEquipment({ ...params, sorter, filter })}
