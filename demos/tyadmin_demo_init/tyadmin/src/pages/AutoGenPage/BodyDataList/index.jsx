...
import { addBodyData, queryBodyData, removeBodyData, updateBodyData, queryBodyDataVerboseName, queryBodyDataListDisplay, queryBodyDataDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateBodyData(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryBodyDataDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryBodyDataListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryBodyData, table_columns, 'body data-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索body data" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'body data-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryBodyData({ ...params, sorter, filter })}
